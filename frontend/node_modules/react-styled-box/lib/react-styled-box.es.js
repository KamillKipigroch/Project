...
function toCss(props, propTypes, propsMap) {
  return Object.keys(propTypes).filter(function (k) {
    ...
    return "".concat(propsMap[k].name, ": ").concat(propsMap[k].apply(props[k]), ";");
  }).join('');
}
  ...
  return function (props, propName, componentName) {
    if (process.env.NODE_ENV !== 'production') {
      ...
      if (props['display'] !== 'flex' && props['display'] !== 'inline-flex') {
        return new Error("The property \"".concat(propName, "\" in ").concat(componentName, " is available only when is display={/flex|inline-flex/}."));
      }
...
function defaultPx(prop) {
  return typeof prop === 'number' ? "".concat(prop, "px") : prop;
}
...
var flexFlowValues = flexDirectionValues.map(function (direction) {
  return flexWrapValues.map(function (wrap) {
    return "".concat(direction, " ").concat(wrap);
  });
