...
const UserOrdersDataGrid = () => {
  const { orderStore, opinionStore, productStore } = useStores();
            ...
            {opinionStore.checkIfOpinionCanBeAdded(params.row.productID) ? (
              ...
              <>
                <Tooltip title={t("editOpinion")} arrow={true}>
                  <IconButton
                    onClick={() =>
                      opinionStore.openPopup(params.row, params.row.id)
                    }
                  >
                    <EditIcon sx={{ color: "#4f70e8" }} />
                  </IconButton>
                </Tooltip>
                <Tooltip title={t("deleteOpinion")} arrow={true}>
                  <IconButton
                    onClick={() =>
                      opinionStore.disableVisibility(params.row)
                    }
                  >
